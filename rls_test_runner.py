...
Requirements: Python 3.8+, psycopg2. No other packages. YAML definitions are
...
try:
    import rls_kit
    from rls_kit import definitions as defs
    from rls_kit import db as dbmod
    from rls_kit import engine
    from rls_kit import report as reportmod
except ImportError as exc:  # pragma: no cover
    sys.stderr.write(
        "rls_test_runner: could not import the rls_kit package next to this "
        "script.\n"
        f"  {exc}\n"
        "Run the runner from the directory you unpacked it into, keeping "
        "rls_kit/ beside it, or set PYTHONPATH to that directory.\n"
    )
    raise SystemExit(3)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {rls_kit.__version__} ({rls_kit.__product__})",
    )
    return p
...
def default_definition_paths() -> List[str]:
    candidate = os.path.join(HERE, "tests_definitions")
